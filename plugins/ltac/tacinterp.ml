  ...
  | VFun (appl,trace,loc,lfun,[],t) ->
    Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (_name, poly) ->
      let ist = {
        ...
        extra = TacStore.set ist.extra f_trace (if Profile_tactic.get_profiling() then empty_trace else trace); } in
      let tac = name_if_glob appl (eval_tactic_ist ist t) in
      do_profile trace.stack (catch_error_tac_loc loc trace tac)
