...
let interp_gen kind ist pattern_mode flags env sigma c =
  ...
  let trace = push_trace (loc,LtacConstrInterp (term,vars)) ist in  (* TODO: if trace enabled *)
...
and eval_tactic_ist ist tac : unit Proofview.tactic =
  ...
  | TacAtom t ->
      ...
      Profile_ltac.do_profile "eval_tactic:2" trace.stack
  ...
  | TacAbstract (t,ido) ->
      ...
      Profile_ltac.do_profile "eval_tactic:TacAbstract" trace.stack
...
and interp_ltac_reference ?loc' mustbetac ist r : Val.t Ftactic.t =
  ...
  | ArgArg (loc,r) ->
      ...
      Profile_ltac.do_profile "interp_ltac_reference" trace.stack ~count_call:false
      ...
      if List.is_empty lvar then
        begin wrap_error
            begin
              ...
              Profile_ltac.do_profile "interp_app" trace.stack ~count_call:false
...
and tactic_of_value ist vle =
  ...
  | VFun (appl,trace,loc,lfun,[],t) ->
    Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (_name, poly) ->
      ...
      Profile_ltac.do_profile "tactic_of_value" trace.stack (catch_error_tac_loc loc trace tac)
