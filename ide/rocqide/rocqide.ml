...
let create_session f =
  ...
  sn.rocqops#set_forward_get_other_proof (fun () -> (find_secondary_sn sn).proof);
...
let send_to_rocq_aux name f sn =
  let info () = Minilib.log ("Rocq busy, discarding " ^ name) in
...
module Nav = struct
  let forward_one_sid ?sid _ = maybe_update_breakpoints ();
    if not (resume_debugger ?sid Interface.StepOver) then
      send_to_rocq "forward one" (fun sn -> sn.rocqops#process_next_phrase)
  ...
  let continue ?sid _ = maybe_update_breakpoints ();
    if not (resume_debugger ?sid Interface.Continue) then
      send_to_rocq "continue" (fun sn -> sn.rocqops#process_until_end_or_error)
  ...
  let step_in ?sid _ = maybe_update_breakpoints ();
    if not (resume_debugger ?sid Interface.StepIn) then
      send_to_rocq "step in" (fun sn -> sn.rocqops#process_next_phrase)
  ...
  let step_out ?sid _ = maybe_update_breakpoints ();
    if not (resume_debugger ?sid Interface.StepOut) then
      send_to_rocq "step out" (fun sn -> sn.rocqops#process_next_phrase)
  ...
  let run_to_end _ = maybe_update_breakpoints ();
    send_to_rocq "run to end" (fun sn -> sn.rocqops#process_until_end_or_error)
  ...
  let join_document _ = send_to_rocq "join" (fun sn -> sn.rocqops#join_document)
end
