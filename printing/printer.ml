...
let pr_open_subgoals_diff  ?(quiet=false) ?prev_proof proof =
  ...
  | Some proof ->
    let output =
      if Proof_diffs.show_diffs () then
        try pr_open_subgoals_diff ?prev_proof:oldp proof
...
let diff_pr_open_subgoals ?(quiet=false) o_proof n_proof =
  ...
  | None -> Pp.mt ()
  | Some proof ->
    try pr_open_subgoals_diff ~quiet ?prev_proof:o_proof proof
