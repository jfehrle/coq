...
(* write the diffs in html *)
let write_html_diffs oldp newp loc =
  let diffs = try
    Proof_diffs.diff_pr_open_subgoals ~quiet:true oldp newp
    (* todo: print the unparsable string (if we know it) *)
  with Proof_diffs.Diff_Failure pp -> (Feedback.msg_warning
    (Pp.str "Diff failure; showing results without diff highlighting" );
    pp) in
  try
